...
export async function GET(req: Request, context: any) {
  try {
    ...
    // Resolve numeric user_id
    let userId = await resolveUserIdViaLink(norm, supa);
    let edges: any[] = [];
    let source = 'reels:user_id';

    // Try resolving user ID from host endpoints if not found
    if (!userId) {
      const infoEndpoints = [
      ...
      // Persist to cache if resolved here
      if (userId) {
        try { await supa.from('instagram_user_ids').upsert({ instagram_username: norm, instagram_user_id: String(userId), created_at: new Date().toISOString() }, { onConflict: 'instagram_username' }); } catch {}
      }
    }
    ...
    // Username fallback: profile1
    if (allowUsernameFallback && upserts.length === 0) {
      const medias = await fetchProfileData(norm);
      for (const m of medias) {
        const id = String(m?.id || m?.shortcode || ''); if (!id) continue;
        const ms = parseMs(m?.timestamp) || parseMs(m?.taken_at) || null; if (!ms) continue;
        const post_date = new Date(ms).toISOString().slice(0,10);
    ...
    try {
      const arr = await fetchLinksData(`https://www.instagram.com/${norm}/reels/`);
      for (const it of arr) {
    ...
    for (const e of edges) {
      ...
      if (!ms) {
        ...
        if (code && linksMap.has(code)) { ms = linksMap.get(code)!; telemetry.linkMatches += 1; }
        if (!ms) {
          ms = await resolveTimestamp(media, node);
          if (ms) telemetry.detailResolves += 1;
        }
        if (!ms) { telemetry.skippedNoTimestamp += 1; continue; }
      }
      const d = new Date(ms!);
    ...
    // Extra fallback: profile root links
    if (allowUsernameFallback && upserts.length === 0) {
      try {
        const urls = [
          `https://www.instagram.com/${norm}/`,
          `https://instagram.com/${norm}/`
        ];
        let arr: any[] = [];
        for (const p of urls) {
          const tmp = await fetchLinksData(p);
        ...
        for (const it of arr) {
          const sc = String(it?.shortcode || it?.meta?.shortcode || ''); if (!sc) continue;
          let ms = parseMs(it?.takenAt || it?.meta?.takenAt) || null;
          
          if (!ms) {
            try {
              const info = await rapidApiRequest<any>({ url: `https://${IG_HOST}/api/instagram/post_info?code=${encodeURIComponent(sc)}` , method: 'GET', rapidApiHost: IG_HOST, timeoutMs: 15000 });
              ...
              if (post_date && (views + likes + comments) > 0) upserts.push({ id: sc, username: norm, post_date, play_count: views, like_count: likes, comment_count: comments });
            } catch {}
          } else {
            const post_date = new Date(ms).toISOString().slice(0,10);
            let views = Number(it?.playCount || it?.viewCount || 0) || 0;
    ...
    // Final attempt: scraper provider
    if (upserts.length === 0) {
      try {
        const userId2 = userId || await resolveUserId(norm, supa);
        if (userId2) {
          const sj = await rapidApiRequest<any>({ url: `https://${IG_SCRAPER_HOST}/get_instagram_reels_details_from_id?user_id=${encodeURIComponent(userId2)}`, method: 'GET', rapidApiHost: IG_SCRAPER_HOST, timeoutMs: 20000 });
          const reels: any[] = (sj?.data?.reels || sj?.reels || sj?.data?.items || sj?.items || []) as any[];
          for (const it of reels) {
            const id = String(it?.id || it?.code || ''); if (!id) continue;
            const ms = parseMs(it?.taken_at) || parseMs(it?.device_timestamp) || null; if (!ms) continue;
            const post_date = new Date(ms).toISOString().slice(0,10);
    ...
    if (upserts.length) {
      if (cleanup === 'today') {
        ...
        try { await supa.from('instagram_posts_daily').delete().eq('username', norm).eq('post_date', today); } catch {}
      }
      const chunk = 500;
    ...
    if (ownerUserId) {
      try {
        ...
        handles.add(norm);
        try { const { data: u1 } = await supa.from('users').select('instagram_username').eq('id', ownerUserId).maybeSingle(); if (u1?.instagram_username) handles.add(String(u1.instagram_username).replace(/^@/, '').toLowerCase()); } catch {}
        try { const { data: u2 } = await supa.from('user_instagram_usernames').select('instagram_username').eq('user_id', ownerUserId); for (const r of u2||[]) handles.add(String((r as any).instagram_username).replace(/^@/, '').toLowerCase()); } catch {}
        try { const { data: u3 } = await supa.from('employee_instagram_participants').select('instagram_username').eq('employee_id', ownerUserId); for (const r of u3||[]) handles.add(String((r as any).instagram_username).replace(/^@/, '').toLowerCase()); } catch {}
        if (handles.size > 0) {
          const all = Array.from(handles);
          const winDays = 60; const start = new Date(); start.setUTCDate(start.getUTCDate()-winDays+1); const startISO = start.toISOString().slice(0,10);
