...
export async function GET(req: Request) {
  try {
    ...
    if (mode === 'live') {
      ...
      return NextResponse.json({ top, campaignId, start: campaignStart, end: campaignEnd, prizes, data, source: 'live' });
    }
    ...
    if (mode === 'accrual') {
      ...
      if (!empIds.length) return NextResponse.json({ top, campaignId, start: startISO, end: endISO, prizes: null, data: [], required_hashtags: requiredHashtags, filtered_by_hashtag: requiredHashtags && requiredHashtags.length > 0 });
